 ...
 * @version Nov 1, 2014
 */
public class NinjaAssassinScreen
    extends ShapeScreen
{
    /**
     * Room
    ...
    public void initialize(int lvl)
    {
        ...
        try
        {
            Log.i("Screen", "level" + lvl);
            room = new Room(getResources().getAssets().open("level" + lvl));
        ...
        if (room != null)
        {
            ...
            for (int y = 0; y < room.getTileImages().length; y++)
            {
                for (int x = 0; x < room.getTileImages()[y].length; x++)
                {
                    ...
                    Tile tile =
                        new Tile(
                            room.getTileImages()[y][x],
                            left,
                            top,
                            right,
                            bottom,
                            TileType.getTileType(room.getTileImages()[y][x]));
            ...
            for (Entity entity : room.getEntities())
            {
                if (entity instanceof Enemy)
                {
                    ...
                    EntityDrawing i =
                        new EntityDrawing("enemy", sideLength, enemy);
                    enemy.addObserver(i);
                    add(i);
                ...
                else if (entity instanceof Target)
                {
                    ...
                    EntityDrawing i =
                        new EntityDrawing("target", sideLength, target);
            ...
            // Create ninja
            ninja = room.getNinja();
            Log.i("Screen", "The starting location is : "
                + ninja.getLocation().toString());
